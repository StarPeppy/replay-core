...
public class RTPA implements CommandExecutor{
	private StarPep plugin;
	public RTPA(StarPep pl){
		plugin = pl;
	}
	@Override
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(args.length != 1){
			player.sendMessage(ChatColor.DARK_AQUA + "/rtpa <player>, or /tpa <player>");
			return false;
		}
		Player target = null;
		...
		if(!(player.hasPermission("replaycore.rtpa"))){
		...
		target.sendMessage(ChatColor.AQUA + "Teleport request recieved from " + ChatColor.BLUE + player.getName() + ".");
		...
		List<String> pendingForTarget = plugin.getConfig().getStringList("Players.PendingRequestsTPA." + target.getUniqueId().toString());
		List<String> pendingForTargetHere = plugin.getConfig().getStringList("Players.PendingRequestsHereTPAHERE." + target.getUniqueId().toString());
		if(pendingForTargetHere.contains(player.getName())){
			pendingForTargetHere.remove(player.getName());
